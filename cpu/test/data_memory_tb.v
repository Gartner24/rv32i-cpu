...
reg        mem_write, mem_read, en;
reg [31:0] addr, write_data;
wire[31:0] read_data;

data_memory dut (
    .mem_write(mem_write), .mem_read(mem_read), .en(en),
...
initial begin
    errors = 0;
    mem_write=0; mem_read=0; en=1;
    addr=0; write_data=0;

    // Write to word 0, read back
    addr=32'd0; write_data=32'hDEADBEEF; mem_write=1; #1;
    ...
    addr=32'd4; write_data=32'hCAFEBABE; mem_write=1; #1;
    mem_write=0; mem_read=1; #1;
    ...
    // en=0 blocks write (pre-load word8=0xAAAAAAAA, then try to overwrite)
    en=1; addr=32'd8; write_data=32'hAAAAAAAA; mem_write=1; #1;
    en=0; write_data=32'h12345678; #1;
    mem_write=0; en=1; mem_read=1; #1;
    if (read_data!==32'hAAAAAAAA) begin $display("FAIL: en=0 should block write got %0h", read_data); errors=errors+1; end

    // en=1 write works again
    en=1; addr=32'd8; write_data=32'hABCD1234; mem_write=1; #1;
    mem_write=0; mem_read=1; #1;
